/* This is a modified version of 
   ...
   To get access to the raw numbers in an XLSX spreadsheet, not the screen-formatted numbers that 
   XSSFSheetXMLHandler produces. 
   
   Changes:
     1. See section near line 368, case NUMBER:
     2. Class name and references changed from XSSFSheetXMLHandler to XSSFSheetXMLHandlerNumeric
     3. Package name changed, to put this in the VIMC namespace.
     4. Also changed some references of "CellRef" to "_CellRef" to avoid hiding the class variable. Personal preference to remove warning...
     
   Wes Hinsley, MRC Centre for Outbreak Analysis & Modelling
...
==================================================================== */
package com.vimc.demography.tools; // Changed 
 ...
 */
public class XSSFSheetXMLHandlerNumeric extends DefaultHandler {
    private static final POILogger logger = POILogFactory.getLogger(XSSFSheetXMLHandlerNumeric.class);
    ...
    */
   public XSSFSheetXMLHandlerNumeric(
           StylesTable styles,
   ...
   private void checkForEmptyCellComments(EmptyCellCommentsCheckType type) {
       if (commentCellRefs != null && !commentCellRefs.isEmpty()) {
           ...
           do {
               CellAddress _cellRef = new CellAddress(this.cellRef);
               CellAddress peekCellRef = commentCellRefs.peek();
               if (type == EmptyCellCommentsCheckType.CELL && _cellRef.equals(peekCellRef)) {
                   ...
                   int comparison = peekCellRef.compareTo(_cellRef);
    ...
    */
   private void outputEmptyCellComment(CellAddress _cellRef) {
       XSSFComment comment = commentsTable.findCellComment(_cellRef);
       output.cell(_cellRef.formatAsString(), null, comment);
   }
