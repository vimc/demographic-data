...
public class InterpolatedDemographicIndicators {
  ...
  private int[] raw_data = new int[no_countries*no_years*no_fields]; // 201 countries * 150 years * 17 fields
  ...
  public static final byte DEATHS_BOTH = 0;                        // number        
  public static final byte DEATHS_MALE = 1;                        // number
  public static final byte DEATHS_FEMALE = 2;                      // number    
  public static final byte CRUDE_DEATH_RATE = 3;                   // 1000s deaths per 1000 population
  public static final byte LIFE_EXPECTANCY_AT_BIRTH_BOTH = 4;      // years * 1000 !
  public static final byte LIFE_EXPECTANCY_AT_BIRTH_MALE = 5;      // years * 1000
  public static final byte LIFE_EXPECTANCY_AT_BIRTH_FEMALE = 6;    // years * 1000
  public static final byte INFANT_DEATHS = 7;                      // number. (under age 1)
  public static final byte INFANT_MORTALITY_RATE = 8;              // 1000s deaths per 1000s live births
  public static final byte UNDER_5_MORTALITY = 9;                  // number. (under age 5)
  public static final byte BIRTHS = 10;                            // number.
  public static final byte CRUDE_BIRTH_RATE = 11;                  // 1000s per 1000 population
  public static final byte TOTAL_FERTILITY = 12;                   // Live births per woman * 1000!
  public static final byte TOTAL_POPULATION_NATURAL_CHANGE = 13;   // number. (actual increase/decrease of population)
  public static final byte RATE_OF_NATURAL_INCREASE = 14;          // 1000s of people increasing, per 1000 in population
  public static final byte POPULATION_CHANGE = 15;                 // number. (different to 13 - check defs)
  public static final byte POPULATION_GROWTH_RATE = 16;            // Percentage * 1000
  
  private void setDataUnchecked(byte field, int c_index, int year, int value) {
    year-=1950;
    raw_data[field+(year*no_fields)+(c_index*year*no_fields)]=value;
  }
  
  private int getDataUnchecked(byte field, int c_index, int year) {
    year-=1950;
  ...
  public int get(String c3, byte field, int year) {
        ...
        if (row_no>=FIRST_DATA_ROW) {
          ...
          if (last_country_index>=0) {
            ...
            for (byte field=0; field<no_fields; field++) {
              setDataUnchecked(field,last_country_index,year,(int) (Float.parseFloat(bits[6+field])*1000.0f));
            }
          }
        }
      }
