package com.vimc.demography.unwpp2012;
...
public class InterpolatedPopulation {
  ...
  /**   The 2012 data for age-decomposed interpolated population is in three XLS files.
   *    In 1000s of people.
   *   
   *    1950 - 2015 is in the "ESTIMATES" sheet
   *    2015 - 2100 is in the "MEDIUM FERTILITY" sheet - (different name from 2015)
  ...
  private final static String[] xls_files = new String[] {
      "WPP2012_INT_F03_1_POPULATION_BY_AGE_ANNUAL_BOTH_SEXES.XLS",
      "WPP2012_INT_F03_2_POPULATION_BY_AGE_ANNUAL_MALE.XLS",
      "WPP2012_INT_F03_3_POPULATION_BY_AGE_ANNUAL_FEMALE.XLS"};
    
  private final static String SHEET_ESTIMATES = "ESTIMATES";
  private final static String SHEET_MEDIUM = "MEDIUM FERTILITY";
      ...
      if ((current_sheet.equals(SHEET_ESTIMATES)) || (current_sheet.equalsIgnoreCase(SHEET_MEDIUM))) {
        if (row_no>=FIRST_DATA_ROW) {
          ...
          if (!bits[XLSX_COUNTRY_COL].equals(last_country_code)) {             // Change of country
            int next_country_code=(int)Float.parseFloat(bits[XLSX_COUNTRY_COL]);    // This is the integer code. (Could be <3 digits)
            if (next_country_code<900) {                                       // We want country, not WORLD/CONTINENT.
              
              // Have we met this country before?  
            
              last_country_code=((next_country_code<10)?"0":"")+((next_country_code<100)?"0":"")+(int)Float.parseFloat(bits[XLSX_COUNTRY_COL]);  // Insert missing 0s
          ...
          if (last_country_index>=0) {
            ...
            int year = (int)Float.parseFloat(bits[XLSX_YEAR_COL]);
        ...
        if (pick_country) {
          for (byte g=0; g<no_genders; g++) {
            ...
            for (int y=1950; y<=1989; y++) {
              for (int a=0; a<=79; a++) {
                huge_statement.append("("+
                                   a+","+a+","+get(a,i3,g,y)+",'"+y+"-07-01','"+(y+1)+"-06-30','"+MontaguDB.projection_ids[MontaguDB.UNWPP_ESTIMATES]+"','"+gg+"','"+c3+"','"+MontaguDB.demographic_statistic_types_ids[MontaguDB.INT_POP]+"','"+MontaguDB.source_ids[MontaguDB.UNWPP_2012]+"'),");
              
              }
              huge_statement.append("("+
                  "80,120,"+get(80,i3,g,y)+",'"+y+"-07-01','"+(y+1)+"-06-30','"+MontaguDB.projection_ids[MontaguDB.UNWPP_ESTIMATES]+"','"+gg+"','"+c3+"','"+MontaguDB.demographic_statistic_types_ids[MontaguDB.INT_POP]+"','"+MontaguDB.source_ids[MontaguDB.UNWPP_2012]+"'),");

            }
            for (int y=1990; y<=2100; y++) {
              String proj=(y<=2015)?MontaguDB.projection_ids[MontaguDB.UNWPP_ESTIMATES]:MontaguDB.projection_ids[MontaguDB.UNWPP_MEDIUM];
              for (int a=0; a<=99; a++) {
                huge_statement.append("("+
                    a+","+a+","+get(a,i3,g,y)+",'"+y+"-07-01','"+(y+1)+"-06-30','"+proj+"','"+gg+"','"+c3+"','"+MontaguDB.demographic_statistic_types_ids[MontaguDB.INT_POP]+"','"+MontaguDB.source_ids[MontaguDB.UNWPP_2012]+"'),");

              }
              huge_statement.append("("+
                  "100,120,"+get(100,i3,g,y)+",'"+y+"-07-01','"+(y+1)+"-06-30','"+proj+"','"+gg+"','"+c3+"','"+MontaguDB.demographic_statistic_types_ids[MontaguDB.INT_POP]+"','"+MontaguDB.source_ids[MontaguDB.UNWPP_2012]+"'),");
            }
          }
        }
      }
  ...
  public InterpolatedPopulation(String path, Element _iso3166) throws Exception {
    ...
    for (byte gender=0; gender<3; gender++) { 
      VIMC_XLSDocumentParser.parse(path+(path.endsWith(File.separator)?"":File.separator)+xls_files[gender], new LineParser(gender));
    }
  }
